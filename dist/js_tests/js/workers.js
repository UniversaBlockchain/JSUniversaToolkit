...
(function () {
  ...
  Unitest.addTestModule(function () {
    var APP_TOKEN, CONTRACT_BIN, CONTRACT_KEY, PACKED_CHANGING_ACC_KEY, PACKED_PRIVATE_KEY_1, PACKED_PRIVATE_KEY_2;
    ...
    Unitest.addTest('w1', function () {
      return Universa.WorkerTests.test1();
    });
    Unitest.addTest('w2', function () {
      return Universa.WorkerTests.test2();
    });
    Unitest.addTest('w3', function () {
      return Universa.WorkerTests.test3();
    });
    Unitest.addTest('w4', function () {
      return Universa.WorkerTests.test4();
    });
    Unitest.addTest('w5', async function () {
      var chatleApi, i, isRegistered, isRemoved, items, j, k, l, limit, m, messages, messagesCount, msg, n, party, pong, privateKey, publicKey, result, results;
      Universa.logger.log('w5() started');
      messagesCount = 0;
      Universa.logger.log("Creating a key pair...");
      privateKey = await Universa.PrivateKey.create(2048);
      publicKey = privateKey.publicKey;
      Universa.logger.log("Connecting...");
      chatleApi = await CryptoCloud.connect(APP_TOKEN, privateKey);
      isRegistered = await chatleApi.registerNick("myNickExample", true);
      Universa.logger.log("Checking a connection...");
      pong = await chatleApi.ping();
      Universa.logger.log(JSON.stringify(pong));
      Universa.logger.log("Registering a nick...");
      isRemoved = await chatleApi.registerNick("myNickExample");
      Universa.logger.log("Setting a password...");
      result = await chatleApi.setPassword("myPasswordExample");
      Universa.logger.log("Getting a party...");
      ...
      for (j = l = 0; l < 10; j = ++l) {
        messages = [];
        for (i = m = 0; m < 100; i = ++m) {
          msg = "Hello " + messagesCount;
          Universa.logger.log(`Delivering the message '${msg}'...`);
          messages.push(party.newMessage(msg).deliver());
          messagesCount = messagesCount + 1;
          if (messages.length >= 10) {
            Universa.logger.log("Awaiting delivery tasks...");
        ...
        messages = await Promise.all(messages);
        for (k = n = 0; n < 1; k = ++n) {
          limit = Math.min(messagesCount, (k + 1) * 1000);
          Universa.logger.log(`Loading items (MessagesCount: ${messagesCount}, Limit: ${limit})...`);
          items = await chatleApi.loadItems({
            tags: ["comm"],
            limit: limit
          });
        }
        Universa.logger.log("Items loaded");
        results.push(console.log(items));
      }
      return results;
    });
    Unitest.addTest('w6', function () {
      return Universa.WorkerTests.test6();
    });
    Unitest.addTest('w7', function () {
      return Universa.WorkerTests.test7();
    });
    Unitest.addTest('w8', function () {
      return Universa.WorkerTests.test8();
    });
    Unitest.addTest('w9', function () {
      return Universa.WorkerTests.test9();
    });
    return Unitest.addTest('w10', function () {
      return Universa.WorkerTests.test10();
    });
  });
}).call(this);