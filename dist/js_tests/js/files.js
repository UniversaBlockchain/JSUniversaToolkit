// Generated by CoffeeScript 2.3.1
(function () {
  var dec64, enc64;

  dec64 = Universa.utils.decode64;

  enc64 = Universa.utils.encode64;

  Unitest.addTestModule(function () {
    var APP_TOKEN, PACKED_PRIVATE_KEY_1, PACKED_PRIVATE_KEY_2;
    APP_TOKEN = "ehUEdnfhDqhBAWK77Rndam0d+L6NVhGDGm/ma3SHHZQA4kgs4uf1AnYJKqKHU2qQZchSeieZyquGOuCHeUtULw==";
    PACKED_PRIVATE_KEY_1 = dec64('');
    PACKED_PRIVATE_KEY_2 = dec64('');
    Unitest.addTest('zip and unzip single file', async function () {
      var api, archive, data, file, filecontent, id, unzipFiles;
      api = await CryptoCloud.connect(APP_TOKEN, PACKED_PRIVATE_KEY_1);
      filecontent = "Hello World";
      data = Universa.utils.v2.asByteArray(filecontent);
      file = FileInfo(data, "hello.txt");
      archive = Universa.Archive.FromFiles(Array(file));
      id = await archive.ToCloud(api);
      console.log('item created!');
      console.log(id);
      this.assert(id > 0);
      unzipFiles = await Universa.Archive.FromCloud(id, api);
      console.log(unzipFiles);
      this.assertEquals(unzipFiles.fileInfos[0].name, 'hello.txt');
      return this.assertEquals(String.fromCharCode.apply(null, unzipFiles.fileInfos[0].content), filecontent);
    });
    return Unitest.addTest('zip and unzip multi files', async function () {
      var api, archive, data, file1, file2, filecontent, id, unzipFiles;
      api = await CryptoCloud.connect(APP_TOKEN, PACKED_PRIVATE_KEY_1);
      filecontent = "Hello World222";
      data = Universa.utils.v2.asByteArray(filecontent);
      file1 = FileInfo(data, "hello.txt");
      file2 = FileInfo(data, "hello2.txt");
      archive = Universa.Archive.FromFiles(Array(file1, file2));
      id = await archive.ToCloud(api);
      console.log('item created!');
      console.log(id);
      this.assert(id > 0);
      unzipFiles = await Universa.Archive.FromCloud(id, api);
      console.log(unzipFiles);
      this.assertEquals(unzipFiles.fileInfos[0].name, 'hello.txt');
      this.assertEquals(String.fromCharCode.apply(null, unzipFiles.fileInfos[0].content), filecontent);
      this.assertEquals(unzipFiles.fileInfos[1].name, 'hello2.txt');
      return this.assertEquals(String.fromCharCode.apply(null, unzipFiles.fileInfos[1].content), filecontent);
    });
  });
}).call(this);