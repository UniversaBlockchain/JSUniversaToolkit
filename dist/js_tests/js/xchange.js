...
(function () {
  var APP_TOKEN, Contract, dec64, enc64;
  ...
  Unitest.addTestModule(function () {
    var EXISTING_CONVERSION_ORDER_CODE, EXISTING_PRODUCT_ORDER_CODE, PACKED_PRIVATE_KEY_2;
    ...
    Unitest.addTest('get utn-u limits', async function () {
      var response;
      response = await XChange.utn.u.getLimits();
      console.log(response);
      return this.assertEquals(response.status, 'OK');
    });
    Unitest.addTest('create custom utn-u order', async function () {
      var UTNcontract, cryptoCloudAPI, nodeApi, order;
    ...
    Unitest.addTest('create utn-u order', async function () {
      var amount, compound, order, paymentContract, privateKey, privateKeyBIN, response, tpack, type, uPackBIN, utnBIN;
    ...
    Unitest.addTest('get utn-utnp limits', async function () {
      var response;
      console.log("RUN");
    ...
    Unitest.addTest('create utn-utnp conversion', async function () {
      var compound, order, privateKey, privateKeyBIN, response, returnAddress, tpack, type, uPackBIN, utnBIN;
      privateKeyBIN = dec64('');
      ...
      return this.assertEquals('OK', 'OK');
    });
    Unitest.addTest('get products list', async function () {
      var response;
      response = await XChange.store.getProducts();
      return this.assertEquals(response.status, 'OK');
    });
    Unitest.addTest('get conversions list', async function () {
      var response;
      response = await XChange.store.getConversions();
      console.log(response);
      return this.assertEquals(response.status, 'OK');
    });
    Unitest.addTest('make product order', async function () {
      var currency, downloaded, order, privateKey, product, productCode, publicKey, quantity, response, returnAddress, stateResponse;
      response = await XChange.store.getProducts();
      ...
      this.assertTruthy(order.orderCode);
      stateResponse = await order.getState();
      this.assertEquals(stateResponse.status, "OK");
      this.assertEquals(stateResponse.orderState.status, "awaits_payment");
      downloaded = await order.download();
      return this.assertEquals(downloaded.status, "error");
    });
    Unitest.addTest('make conversion order', async function () {
      var amount, currency, downloaded, order, privateKey, publicKey, response, returnAddress, stateResponse, type;
      response = await XChange.store.getConversions();
      returnAddress = "0x8A5A90415Fea7f4ECfC2eF37d1873Ca0B239C0aD";
    ...
    Unitest.addTest('make utnp-utn conversion order', async function () {
      var amount, order, privateKey, publicKey, response, state, type;
      ...
      console.log(state);
      return this.assertEquals(state.utnpImport.status, "wait_payment");
    });
    // when state is "ready", here will be utn_contract_base64 key
    Unitest.addTest('find product order', async function () {
      var api, currency, foundOrder, itemId, order, order2, orderCode, privateKey, productCode, publicKey, quantity, returnAddress, stateResponse;
      productCode = '1';
      privateKey = Universa.PrivateKey.fromBOSS(PACKED_PRIVATE_KEY_2);
      publicKey = privateKey.publicKey;
      quantity = 1;
      ...
      this.assertEquals(foundOrder.productCode, "1");
      //# cloud operations
      APP_TOKEN = "ehUEdnfhDqhBAWK77Rndam0d+L6NVhGDGm/ma3SHHZQA4kgs4uf1AnYJKqKHU2qQZchSeieZyquGOuCHeUtULw==";
      api = await CryptoCloud.connect(APP_TOKEN, PACKED_PRIVATE_KEY_2);
      itemId = await XChange.Order.ToCloud(order, api);
      console.log(itemId);
      this.assert(itemId > 0);
      order2 = await XChange.Order.FromCloud(itemId, api);
      return this.assertEquals(order2.productCode, "1");
    });
    Unitest.addTest('check product order (static)', async function () {
      var response;
      response = await XChange.Order.getState(EXISTING_PRODUCT_ORDER_CODE, "product");
      console.log(response);
      return this.assertEquals(response.status, 'OK');
    });
    Unitest.addTest('download product order (static)', async function () {
      var response;
      response = await XChange.Order.download(EXISTING_PRODUCT_ORDER_CODE, "product");
      return this.assertEquals(response.status, 'error');
    });
    Unitest.addTest('check conversion order (static)', async function () {
      var response;
      response = await XChange.Order.getState(EXISTING_CONVERSION_ORDER_CODE, "conversion");
      console.log(response);
      return this.assertEquals(response.status, 'OK');
    });
    return Unitest.addTest('download conversion order (static)', async function () {
      var response;
