...
(function () {
  ...
  Unitest.addTestModule(function () {
    var APP_TOKEN, CONTRACT_BIN, CONTRACT_BIN2, CONTRACT_KEY, PACKED_PRIVATE_KEY_1, PACKED_PRIVATE_KEY_2, U_CONTRACT_BIN, U_CONTRACT_BIN2, U_KEY;
    ...
    Unitest.addTest('check Capsule.getInfo', function () {
      var contract, privKey, pubKey, pubKey2, result;
      contract = Contract.fromBOSS(CONTRACT_BIN2);
      result = contract.temp.getInfo();
      console.log(result);
      this.assertEquals(result.publicKeys.length, 1);
      this.assertEquals(result.roles.length, 3);
      pubKey = result.publicKeys[0];
      this.assertTrue(contract.temp.isSignedBy(pubKey));
      privKey = Universa.PrivateKey.fromBOSS(CONTRACT_KEY);
      pubKey2 = privKey.publicKey;
      return this.assertFalse(contract.temp.isSignedBy(pubKey2));
    });
    Unitest.addTest('check Capsule.getPermissions', function () {
      var contract, privKey, pubKey, result;
      privKey = Universa.PrivateKey.fromBOSS(CONTRACT_KEY);
    ...
    Unitest.addTest('check isRoleAvailable method', function () {
      var contract, privKey, pubKey, roleAvailabilityResult;
      privKey = Universa.PrivateKey.fromBOSS(CONTRACT_KEY);
      pubKey = privKey.publicKey;
      contract = Contract.createToken(pubKey, '100', 0.1, "My test token", "MTK", "description", true);
      roleAvailabilityResult = contract.temp.isRoleAvailable('owner', [pubKey]);
      this.assertTrue(roleAvailabilityResult);
      roleAvailabilityResult = contract.temp.isRoleAvailable('another role', [pubKey]);
      this.assertFalse(roleAvailabilityResult);
      roleAvailabilityResult = contract.temp.isRoleAvailable('another role', []);
      return this.assertFalse(roleAvailabilityResult);
    });
    return Unitest.addTest('v2 import alice test all', async function () {
      var chatleAPI, res, res0CloudId;
