...
(function () {
  ...
  Unitest.addTestModule(function () {
    var CONTRACT_KEY, PACKED_PRIVATE_KEY_2;
    ...
    Unitest.addTest('connect to node', async function () {
      var conn, res;
      conn = await Node.connect(Universa.PrivateKey.fromBOSS(CONTRACT_KEY), true);
      res = await conn.command('sping');
      return this.assertEquals('spong', res.sping);
    });
    return Unitest.addTest('check contract', async function () {
      var conn, contractId, res;
      ...
      return this.assertEquals(res.itemResult.state, 'APPROVED');
    });
  });
}).call(this);