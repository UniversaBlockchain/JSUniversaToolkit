// Generated by CoffeeScript 2.3.1
(function () {
  var Contract, dec64, enc64, test;

  test = window.unitjs || {};

  dec64 = Universa.utils.v2.decode64;

  enc64 = Universa.utils.v2.encode64;

  Contract = Universa.Contract;

  Unitest.addTestModule(function () {
    var APP_TOKEN, CONTRACT_KEY, PACKED_PRIVATE_KEY_1, PACKED_PRIVATE_KEY_2;
    PACKED_PRIVATE_KEY_1 = dec64('');
    PACKED_PRIVATE_KEY_2 = dec64('');
    CONTRACT_KEY = dec64("");
    APP_TOKEN = "ehUEdnfhDqhBAWK77Rndam0d+L6NVhGDGm/ma3SHHZQA4kgs4uf1AnYJKqKHU2qQZchSeieZyquGOuCHeUtULw==";
    Unitest.addTest('add files to contract', async function () {
      var api, cloudId, contract, data, data2, file1, file2, filecontent, filecontent2, files, files1, files2, localContractIds, privKey, pubKey, tempUuid;
      privKey = Universa.PrivateKey.fromBOSS(CONTRACT_KEY);
      pubKey = privKey.publicKey;
      contract = Universa.NotaryContract.create(pubKey, false);
      api = await CryptoCloud.connect(APP_TOKEN, PACKED_PRIVATE_KEY_1);
      filecontent = "Hello World";
      data = Universa.utils.v2.asByteArray(filecontent);
      file1 = FileInfo(data, "hello.txt");
      filecontent2 = "Hello World222";
      data2 = Universa.utils.v2.asByteArray(filecontent2);
      file2 = FileInfo(data2, "hello2.txt");
      files = [file1, file2];
      this.assertFalse(contract.checkFilesInContract(files));
      cloudId = await contract.addFiles(files, api);
      this.assertTrue(cloudId > 0);
      //contract.temp.getDefinition("data.files")
      files1 = [file1];
      files2 = [file1, file2];
      this.assertFalse(contract.checkFilesInContract(files1));
      this.assertTrue(contract.checkFilesInContract(files2));
      tempUuid = enc64(contract.temp.hashId());
      return localContractIds = Universa.Contract.findById(tempUuid);
    });
    //console.log(localContractIds)
    return Unitest.addTest('add and remove files', async function () {
      var api, cloudId1, cloudId2, cloudId3, contract, data, data2, data3, file1, file2, file3, filecontent, filecontent2, filecontent3, files, privKey, pubKey;
      privKey = Universa.PrivateKey.fromBOSS(CONTRACT_KEY);
      pubKey = privKey.publicKey;
      contract = Universa.NotaryContract.create(pubKey, false);
      api = await CryptoCloud.connect(APP_TOKEN, PACKED_PRIVATE_KEY_1);
      filecontent = "Hello World";
      data = Universa.utils.v2.asByteArray(filecontent);
      file1 = FileInfo(data, "hello.txt");
      filecontent2 = "Hello World222";
      data2 = Universa.utils.v2.asByteArray(filecontent2);
      file2 = FileInfo(data2, "hello2.txt");
      files = [file1, file2];
      cloudId1 = await contract.addFiles(files, api);
      this.assertTrue(cloudId1 > 0);
      this.assertTrue(contract.checkFilesInContract(files));
      filecontent3 = "Hello World333";
      data3 = Universa.utils.v2.asByteArray(filecontent3);
      file3 = FileInfo(data3, "hello3.txt");
      cloudId2 = await contract.addFiles([file3], api);
      this.assertTrue(cloudId2 > cloudId1);
      this.assertFalse(contract.checkFilesInContract(files));
      this.assertTrue(contract.checkFilesInContract([file1, file2, file3]));
      //check remove files
      cloudId3 = await contract.removeFiles(["hello.txt"], api);
      this.assertTrue(cloudId3 > cloudId2);
      this.assertFalse(contract.checkFilesInContract(files));
      return this.assertTrue(contract.checkFilesInContract([file2, file3]));
    });
  });
}).call(this);