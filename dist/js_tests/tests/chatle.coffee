...
Unitest.addTestModule ->
  ...
  Unitest.addTest 'node connection on aggressive requesting', ->
  ...
  Unitest.addTest 'loadCloudOrders', ->
  ...
  Unitest.addTest 'nickToToken', ->
    token = enc64 CryptoCloud.nickToToken('aaaa')
    @assertEquals 'wYIT7Gx6GcXglGZXB6+sJSA8s8Pxdrb6RLU5sG6mrfbNl0fw', token

  Unitest.addTest 'connect', ->
    await connection1

  Unitest.addTest 'ping', ->
  ...
  class TestItem extends CryptoCloud.Item
    constructor: (api) ->
      super(api)

      service = this.service

      this.capsuleSetup((cap) ->
        cap.addEncryptor(service.privateKey.publicKey)
        cap
      )

  testItemConstructor = (api) -> new TestItem(api)

  Unitest.addTest 'create/get/update/delete item', ->
    ...
    @assertFalsey i.record
    await i.save()

    @assert i?.record?.id > 0
    @assert i?.record?.serial > 0
    @assertEquals i?.tag, 'test_01'
    @assertEquals i?.tag, 'test_01'
    console.log 'item created!'


    a2 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    i1 = await a2.getItem testItemConstructor, id: i.id

    @assertEquals "foobarbaz", i1.privateData.foo
    console.log 'item checked!'

    i.setPrivateData("foo", 'bar')
    await i.save()
    await i1.reload()
    @assertEquals "bar", i1.privateData.foo

    await i1.destroy()
    @assertThrows CryptoCloud.NotFound, ->
      await i.reload()

  Unitest.addTest 'create/get large item', ->
    a1 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    i = new TestItem(a1)
    i.setTag 'test_01'
    ...
    @assertEquals(enc64(PACKED_13KB_DATA), enc64(i1.privateData.archive))

  Unitest.addTest 'new party creates registry', ->
    a1 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    ok = await a1.execute "test_destroy_party"
    a1 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    stk1 = a1.storageKey
    @assert a1.nicks
    a2 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    @assertNotNull a1.registry, "it should create new registry"
    stk2 = a2.storageKey

    @assert enc64(stk1.pack()) == enc64(stk2.pack())

  Unitest.addTest '(un)register nick', ->
    a1 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    await a1.execute "test_destroy_party"
    api = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY

    @assertFalsey api.nicks['__the_foobar__']
    @assertTrue await api.registerNick('__the_foobar__')
    @assertTrue api.nicks['__the_foobar__']
    await api.unregisterNick('__the_foobar__')
    @assertFalsey api.nicks['__the_foobar__']
    @assertFalsey await api.unregisterNick('__the_foobar__')

  Unitest.addTest 'registerNickInUse', ->
    api2 = await connection2
    @assertTrue await api2.registerNick('__nick_2__', true)
    api1 = await connection1
    @assertFalse await api1.registerNick('__nick_2__', true)

  Unitest.addTest 'registerShortNick', ->
    api1 = await connection1
    @assertThrows CryptoCloud.ApiError, ->
      await api1.registerNick('a', true)

  Unitest.addTest 'getBadPartyByNick', ->
    api = await connection1
    @assertNull await api.getParty(nick: '__nonexisting_nick__')

  Unitest.addTest 'v1 password', ->
    conn = await CryptoCloud.connectWithPassword APP_TOKEN, 'alice', 'P@55w0rd'
    @assertNotNull(conn)

  Unitest.addTest 'v1 load items', ->
    conn = await CryptoCloud.connectWithPassword APP_TOKEN, 'alice', 'P@55w0rd'
  ...
  Unitest.addTest 'password', ->
    ...
    @assertNotNull conn.partyId
    @assertNotNull conn.registry.storageKey
    @assertEquals a1.partyId, conn.partyId
    @assert passwordNick of conn.nicks

    pty = await conn.getParty(partyId: conn.partyId)
    k = pty.publicKey
    @assertNotNull(k)
    k = pty.publicKey
    @assertNotNull(k)
  ...
  Unitest.addTest 'register second nick and login with same password', ->
    ...
    @assertNotNull conn2.partyId
    @assertNotNull conn2.registry.storageKey
    @assertEquals a1.partyId, conn2.partyId

  Unitest.addTest 'register new nick and login with pass', ->
    password = 'adminadmin'
    nick = randStr()
    ...
    @assertNotNull chatleAPI.partyId
    @assertNotNull chatleAPI.registry.storageKey
  ...
  Unitest.addTest 'change nick', ->
    ...
    @assertNotNull conn.registry.storageKey
    @assertEquals a1.partyId, conn.partyId
    @assert nick2 of conn.nicks

  Unitest.addTest 'boss deserialization item fail', ->
    a1 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    ok = await a1.execute "test_destroy_party"
    a1 = await CryptoCloud.connect TEST_APP_TOKEN, PACKED_CHANGING_ACC_KEY
    ...
    localOrder = {type: 'UTN-UTNP', data: localOrderData }
    ...
    @assertEquals "35884", i1.privateData.foo.data.code
    console.log 'item checked!'
